        ...
        if type(model) == tuple:
            ...
            with torch.no_grad():
                ...
                if type(interim_inputs) is tuple:
                    ...
                    self.interim_inputs_shape = [
                        i.shape for i in interim_inputs
                    ]
                else:
        ...
        if ranked_outputs is not None and self.multi_output:
            ...
            if output_rank_order == "max":
                _, model_output_ranks = torch.sort(
                    model_output_values, descending=True
                )
        ...
        for i in range(model_output_ranks.shape[1]):
            ...
            if self.interim:
                ...
                for k in range(len(self.interim_inputs_shape)):
                    phis.append(
                        torch.zeros(
                            (X[0].shape[0],) + self.interim_inputs_shape[k][1:],
                            device=self.device,
                            dtype=X[0].dtype,
                        )
                    )
                ...
                for k in range(len(X)):
                    phis.append(
                        torch.zeros(
                            X[k].shape, device=self.device, dtype=X[k].dtype
                        )
                    )
            for j in range(X[0].shape[0]):
                ...
                joint_x = [
                    torch.cat((tiled_X[l], self.data[l]), dim=0)
                    for l in range(len(X))
                ]
...
def maxpool(module, grad_input, grad_output):
    ...
    delta_in = (
        module.x[: int(module.x.shape[0] / 2)]
        - module.x[int(module.x.shape[0] / 2) :]
    )
